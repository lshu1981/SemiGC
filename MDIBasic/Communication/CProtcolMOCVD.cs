...
　　　　　　    new　AutoResetEvent(false)//[2]退出线程
    ...
    //真空硅901P
    class CProtcol901P : CStation
    {
        public string sReMark = ""; //接收报文特征码
        public CProtcol901P()
            : base()
        { }
        ...
        public void PacketLoopMsg(CMessage cMsg)
        {
            ...
            string str2 = Address + cMsg.Function;
        ...
        public override bool PortDataRecv(CMessage sSend, CRecvBuff cRecv)
        {
            try
            {
                ...
                if (iMark >= 0)
                {
                    int iFF = sRecv.IndexOf(";FF", iMark);
                    string sValue = sRecv.Substring(iMark + Address.Length + 3, iFF - iMark - Address.Length-3);
                    lock (this)
                    {
                        cRecv.DelBuff(iFF + 3);
                    }
        ...
        private bool GetVarValue(CMessage nMsg, string sValue)
        {
            lock (this)
            {
                try
                {
                    ...
                    {
                        if (nVar.StrTag[0] == nMsg.Function)
                        {
                            Single sinValue = Convert.ToSingle(sValue);
                            nVar.SetValue(sinValue);
                            break;
                        }
                    }
                    return true;
...
　　　　　　    new　AutoResetEvent(false)//[2]退出线程
    ...
    //恒温槽RE215
    class CProtcolRE215 : CStation
    {
        public CProtcolRE215()
            : base()
        { }
